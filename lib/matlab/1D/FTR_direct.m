function [phi] =  FTR_direct(snapshots, front, rank, maxiter)
    phi = zeros(size(snapshots));
    norm_q = norm(snapshots,'fro');
    alpha = 1;
    for i = 1:maxiter
        res = snapshots - front(phi);
%         fun = @(a) g(a,phi,snapshots,rank,front);
%         alpha = fminsearch(fun,alpha)
        phi = phi - alpha*res;
        [U,S,V] = svds(phi,rank);
        phi = U*S*V';
        fprintf("[%d] rel_err %e\n",i, norm(res,'fro')/norm_q)
    end
end

function val = g(alpha, phi0,snapshots, rank,front)
        res = snapshots - front(phi0);
        phi = phi0 - alpha*res;
        [U,S,V] = svds(phi,rank);
        phi = U*S*V';
        val = norm(snapshots - front(phi));
end
...
%       Ennio Condoleo Rome, Italy h: 23.04 9 Jan 2014
    ...
    while (j>0)
        x_new = x+alpha.*d;
        if (f0(x_new)<=f0(x)+gamma*alpha*f1(x)'*d)
            j = 0;
            alpha_armijo = alpha;
        else
            alpha = alpha*delta;
