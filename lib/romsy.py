...
from lib.ROM import generate_reduced_map, solve_ROM, clean_and_split_data, load_reduced_map,clean_data, get_proj_err
from lib.plot_utils import *
from lib.paths import init_dirs
...
########################################################################################################################
########################################################################################################################
def ROMsy(params, dirs={'data':'data', 'images': 'imgs'}, solve_ODE = True, construct_reduced_map = True, solve_galerkin = True, solve_koopman = True, visualize = True):
    """
    REDUCED ORDER MODELLING PROCEDURE
    - This routine computes the Full Order Model Solution (FOM)
    - Splits the data into odd/even snapshots (in the time index) for train and test data
    - Creates a reduced map on the train data
    - With the reduced map a Reduced Order Model (ROM) is created using koopman forecasting or galerkin projections

    Remark:
    - This procedure only "interpolates" in time! For interpolation for different parameters look at ROMsy_param
    ...
    if solve_ODE:
        ...
        X = params.geom.Xgrid
        if visualize: show_animation(qfield,Xgrid= X, frequency = 8, figure_number = 34)
        savemat(data_dir+'/'+case+'_fom.mat',{'fom_solution': q})
    else:
        q=loadmat(data_dir+'/'+case+'_fom.mat')['fom_solution']

    ####################################################
    # 2.) Postprocess clean data and split in train and
    # vaildation set
    ####################################################
    q_train, q_test = clean_and_split_data(params.rom, q)
    ####################################################
    # 3.) Postprocess learn neuronal net or
    # compute FTR (Front Transport Reduction)
    ####################################################
    if construct_reduced_map:
        mapping = generate_reduced_map(params.rom, q_train, data_dir)
    else:
        mapping = load_reduced_map(params.rom, data_dir)

    ##################################################
    # 4.) simulate trajectory using mainfold galerkin [Carlberg paper]:
    ##################################################
    if solve_galerkin:
        ...
        q_tilde = np.reshape(q_tilde,[*params.geom.N,-1])
        lspg_sol_dict = {'fomState_solution': q_tilde,'romState_solution': romState_hist}
        filename = data_dir + '/' + case +'_' + params.rom.mapping_type +'_%ddofs_rom_lspg.mat'%(params.rom.rom_size)
        savemat(filename, lspg_sol_dict)
    else:
        filename = data_dir + '/' + case +'_' + params.rom.mapping_type +'_%ddofs_rom_lspg.mat'%(params.rom.rom_size)
        lspg_sol_dict= loadmat(filename)
    ...
    if solve_koopman:
        ...
        filename = data_dir + '/' + case + '_' + params.rom.mapping_type + '_%ddofs_rom_fourier-koopman.mat' % (params.rom.rom_size)
    ...
    print("\n------------------------------")
    print("cpu-time: \nFOM %.3f s \nGalerkin %.3f \nKoopman %.3f" % (cpu_time_dict["FOM"], cpu_time_dict["ROM-Galerkin"], cpu_time_dict["ROM-Koopman"]))
    print("------------------------------\n")
    return q_train, q_test, lspg_sol_dict, koopman_sol_dict, cpu_time_dict

########################################################################################################################
########################################################################################################################
########################################################################################################################

def ROMsy_params(train_params_list, test_params_list, dirs={'data': 'data', 'images': 'imgs'}, solve_ODE=True, construct_reduced_map=True,
    ...
    if solve_galerkin:
        ...
        lspg_sol_dict = {'fomState_solution': np.reshape(q_tilde,[*params.geom.N,-1]), 'romState_solution': romState_hist}
    ...
    return q_train, q_test, lspg_sol_dict, koopman_sol_dict, info_dict
...
if __name__ == '__main__':
    ...
    q_train, q_test, sol_dict, koopman_sol_dict,  cpu_time_dict = ROMsy_params(train_params_list, test_params_list,
...
# %%
    ...
    if params.dim ==2 :
        ...
        if 'lspg_sol_dict' in locals():
            fig=plt.figure(3)
            fig.suptitle("LSPG")
            i=0
            q_tilde = lspg_sol_dict["fomState_solution"]
            for k in range(0, N, step):
                time = params.rom.time.test_time_points[k]
                i = i + 1
                fig.add_subplot(2, 2, i)
        ...
        a_coef_tilde = lspg_sol_dict["romState_solution"]
    ...
    #################################################################
    qapprox = np.reshape(lspg_sol_dict['fomState_lspg_solution'],[*params.geom.N,-1])
