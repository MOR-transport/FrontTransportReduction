...
def jacobian_rhs_advection_reaction_diffusion_1D_periodic(self, q, t, dq):
    """
    ...
    # right hand side
    rhs_dq = ddq_ddx + 8/delta**2 * (2 * q - 3 * q **2) * dq
...
######################################################################################################################
######################################################################################################################
def rhs_advection_reaction_diffusion_1D_periodic_hyperreduced_numba_wrapper(fom_params, q, t, sampleMeshIndices= None, rhs=None):

    delta = fom_params  .reaction_const
    ...
    rhs_advection_reaction_diffusion_1D_periodic_hyperreduced_numba(q, delta , stencil_list[1][0],stencil_list[1][1], stencil_list[1][2], sampleMeshSize, sampleMeshIndices, rhs)
    # right hand side
    if return_rhs:
        return rhs
...
@njit()
def rhs_advection_reaction_diffusion_1D_periodic_hyperreduced_numba(q, delta,
                                                                    stencil_xx_val, stencil_xx_col, stencil_xx_size,
    ...
    for ir in range(sampleMeshSize):
        ...
        dqxx_i = 0.0
        # dq_dx and dq_dy
        for j in range(stencil_xx_size):
            dqxx_i += stencil_xx_val[stencil_xx_size * index + j] * q[stencil_xx_col[stencil_xx_size * index + j]]

        rhs[ir] = dqxx_i + mu * q_i**2 * ( 1 - q_i )



def jacobian_rhs_advection_reaction_diffusion_1D_periodic_hyperreduced_numba_wrapper(fom_params, q, t, qlin, sampleMeshIndices= None, jac=None):
    """
    ...
    :return:
    """
    assert(True), "to be implemented, stuff below is from 2D"
    ...
    jacobian_rhs_advection_reaction_diffusion_1D_periodic_hyperreduced_numba(q, qlin, ux, uy, a, b, weights,
...
@njit()
def jacobian_rhs_advection_reaction_diffusion_1D_periodic_hyperreduced_numba(q, qlin, ux_hreduced, uy_hreduced, a, b, weights,
...
if __name__ == "__main__":
    ...
    rhs_numba = rhs_advection_reaction_diffusion_1D_periodic_hyperreduced_numba_wrapper(params, q0, t=0)
    # optimized rhs
    tcpu =[0,0]
    tcpu[0] = time()
    rhs_numba = rhs_advection_reaction_diffusion_1D_periodic_hyperreduced_numba_wrapper(params, q0, t=0)
    ...
    # default RHS
    tcpu[1] = time()
    rhs = rhs_advection_reaction_diffusion_1D_periodic(params, q0, t=0)
    tcpu[1] = time() - tcpu[1]
    ...
    tcpu =[0,0]
    tcpu[0] = time()
    rhs_numba = rhs_advection_reaction_diffusion_1D_periodic_hyperreduced_numba_wrapper(params, q0, t=0, sampleMeshIndices=sampleMeshIndices)
    tcpu[0] = time() - tcpu[0]
    # default RHS
    tcpu[1] = time()
    rhs = rhs_advection_reaction_diffusion_1D_periodic(params, q0, t=0)
    ...
    # # run numba once to compile it (otherwise timer counts compilation time)
    # jac_numba = jacobian_rhs_advection_reaction_diffusion_1D_periodic_hyperreduced_numba_wrapper(params, q0, 0, qlin)
    # # optimized rhs
    # tcpu = [0, 0]
    # tcpu[0] = time()
    # jac_numba = jacobian_rhs_advection_reaction_diffusion_1D_periodic_hyperreduced_numba_wrapper(params, q0, 0, qlin)
    ...
    # # default RHS
    # tcpu[1] = time()
    # jac = jacobian_rhs_advection_reaction_diffusion_1D_periodic(params, q0, 0, qlin)
    # tcpu[1] = time() - tcpu[1]
    ...
    params, qsol = solve_FOM(params)

    X,T = np.meshgrid(params.geom.X[0],params.time.t)
    d = params.reaction_const
    phi = (abs(X - 0.5 * params.geom.L[0]) - 2 * T / d - 2)/d
    qtruth = params.front(2*phi)
    err = norm(qtruth-qsol.T)/norm(qtruth)
    print(f"3.) rel error between numerical sol and true solution {err}")