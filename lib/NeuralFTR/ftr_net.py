...
class FTR_Enc(BaseModule):
    def __init__(self, n_alphas=10, spatial_shape=[128, 128], input_norm=nn.InstanceNorm2d, dtype=torch.float32):
        super(self.__class__, self).__init__()
        k = 5
        s = 2
        self.spatial_shape = spatial_shape
        self.input_norm = input_norm(1)
        self.conv1 = nn.Conv2d(in_channels=1, out_channels=8, kernel_size=k, dtype=dtype)
        self.bn1 = nn.BatchNorm2d(8, dtype=dtype)
        self.spatial_shape = [int((n - k)) + 1 for n in self.spatial_shape]
        self.conv2 = nn.Conv2d(in_channels=8, out_channels=16, kernel_size=k, stride=s, dtype=dtype)
        self.bn2 = nn.BatchNorm2d(16, dtype=dtype)
        self.spatial_shape = [int((n - k) / s) + 1 for n in self.spatial_shape]
        self.conv3 = nn.Conv2d(in_channels=16, out_channels=32, kernel_size=k, stride=s, dtype=dtype)
        self.bn3 = nn.BatchNorm2d(32, dtype=dtype)
        self.spatial_shape = [int((n - k) / s) + 1 for n in self.spatial_shape]
        self.conv4 = nn.Conv2d(in_channels=32, out_channels=16, kernel_size=k, stride=s, dtype=dtype)
        self.bn4 = nn.BatchNorm2d(16, dtype=dtype)
        self.spatial_shape = [int((n - k) / s) + 1 for n in self.spatial_shape]
        
        self.out_fc1 = nn.Linear(16 * self.spatial_shape[0] * self.spatial_shape[1], 512, dtype=dtype)
        self.bn_out_fc1 = nn.BatchNorm1d(512, dtype=dtype)
        self.out_fc2 = nn.Linear(512, n_alphas, dtype=dtype)
...
class FTR_Dec(BaseModule):
    def __init__(self, n_alpha=10, f=nn.Sigmoid, learn_frontwidth=False, spatial_shape=[128, 128], lam=1, dtype=torch.float32):
        super(self.__class__, self).__init__()
...
class FTR_Dec_1Lay(BaseModule):
    def __init__(self, n_alpha=10, f=nn.Sigmoid, spatial_shape=[128, 128], lam=1, dtype=torch.float32):
        super(self.__class__, self).__init__()
        self.spatial_shape = spatial_shape
        self.lam = lam
        self.in_fc1 = nn.Linear(n_alpha, self.spatial_shape[0] * self.spatial_shape[1], bias=False, dtype=dtype)
...
class FTR_AE(BaseModule):
    def __init__(self, encoder=None, decoder=None, n_alphas=10, f=nn.Sigmoid, spatial_shape=[128, 128], dtype=torch.float32):
        super(self.__class__, self).__init__()
        self.has_bottleneck = True
        self.encoder = FTR_Enc(n_alphas, spatial_shape=spatial_shape, dtype=dtype) if encoder is None else encoder
        self.decoder = FTR_Dec(n_alphas, f, dtype=dtype) if decoder is None else decoder
