...
from .galerkin import FomAdapter, MyCustomMapper, galerkin_odeint
from .FTR import construct_mapping
...
class rom_params_class:
    ...
    """
    def __init__(self,fom_params, rom_size, fom_size, time_points, online_prediction_method = "galerkin", mapping_type = "POD"):
    ...
    def get_initial_state(self):
        ...
        if self.case == "reaction1D":
            mu_test = self.fom.reaction_const
            initial_rom_state = self.initial_rom_state
        else:
            initial_rom_state = self.sampled_rom_states[:, 0]

        return initial_rom_state
        #
    ...
    def get_sampleMeshIndices(self):
        if self.galerkin.sampling_method == "random":
            random.seed(22123)
            ...
            assert(True), "Method %s not implemented"%self.galerkin.sampling_method

        return sampleMeshIndices

"""
The following mappers are subclasses of the motherclass MyCustomMapper defined in galerkin_utils.py
...
def generate_reduced_map(rom_params, fom_snapshots, data_dir = None):
        from ROM.FTR import simple_FTR as FTR
        ...
        if rom_params.mapping_type == "POD":
            ...
            phi = FTR( np.reshape(fom_snapshots,[*rom_params.fom.geom.N, -1]), f=front, rank=rom_size,
                       tol=rom_params.ftr.tol, max_iter=rom_params.ftr.max_iter, dt=rom_params.ftr.opt_step_width,
                        nt_show=Nsamples//2, plot_step=100000, print_step=1, offset= rom_params.ftr.offset)
...
def set_up_galerkin( train_params_list, test_params_list, mapping):
        from scipy.interpolate import interp1d
        from scipy.optimize import least_squares
        params = train_params_list[0]
        params.rom.galerkin.mapping = mapping
        if params.rom.mapping_type == "POD" or params.rom.mapping_type == "FTR":
            params.rom.sampled_rom_states = mapping.acoef

        if params.case == "reaction1D":
            time_points = train_params_list[0].time.t
            mu_list = [params.reaction_const for params in train_params_list]
            Nt = len(time_points)
            Nmu = len(mu_list)
            a0_list = np.asarray([params.rom.sampled_rom_states[:,i] for i in np.arange(0,Nmu*Nt,Nt) ])
            a0_guess = interp1d(mu_list, a0_list.T)

            for params in test_params_list:
                q0 = params.inicond
                mu_test = params.reaction_const
                residual = lambda a: q0 - mapping.applyMapping(a)
                res = least_squares(residual, a0_guess(mu_test))
                params.rom.initial_rom_state = res.x
...
def galerkin_timesteper(rom_params, mapping):
    ...
    rom_params.galerkin.fom = FomAdapter(rom_params.fom)
    mapper = rom_params.galerkin.mapping

    dt_min = rom_params.fom.time.dt/rom_params.galerkin.rom_timesteps_per_fom_timestep
    rom_params.sampled_rom_states = mapping.acoef
    rom_params.galerkin.mapping = mapping
    initial_rom_state = rom_params.get_initial_state()
    t0 = 0
    romStates_test = galerkin_odeint(rom_params, initial_rom_state, rom_params.time.test_time_points, mapper, t0=t0,
                                     fomReferenceState=None, sampleMeshIndices = sampleMeshIndices)
...
def POD_DEIM(params_list, fom_snapshots, ndeim = None):
    ...
    from FOM.rhs_advection_reaction_diffusion import rhs_advection_reaction_diffusion_2D_periodic as rhs_ARD_2D
